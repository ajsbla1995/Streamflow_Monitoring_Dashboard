...
def get_groundwater_api_json_response(state_name):
    ...
    '''
    Makes API call to labs.waterdata.usgs.gov/sta/v1.1/ for sensor data. When querying for the Things API, we first get a page
    that links to other endpoints that specifically give more details. Therefore we have to use $expand to directly view those 
    results. Useful for Location data where we need to find the geometries, etc. 
    ...
    params_v3 = {
        "$select": "name,properties,@iot.id",
        "$filter": f"properties/state eq '{state_name}'",
        "$resultFormat": "GeoJSON",
         "$expand": """
                        Locations($select=location),
                        Datastreams($select=name,@iot.id),
                        Datastreams($select=name,@iot.id;$expand=ObservedProperty($select=name,description,@iot.id),
                        Observations($select=result,phenomenonTime,@iot.id;$orderby=phenomenonTime desc;$top=1))
    ...
    try:
        response = requests.get(base_url, params = params_v3, headers=headers)
...
def json_to_gdf(json_response):
    ...
    # Iterate through "features" in the response
    for feature in json_response.get('features', []):
        properties = feature.get('properties', {})
        thing_id = properties.get('@iot.id')
        thing_description = properties.get('properties/monitoringLocationType') 
        thing_name = properties.get('name')
        loc_coords = feature.get('geometry', {}).get('coordinates', None)

        if loc_coords:
            # Process Datastreams
            for i in range(3):  # Assuming max 3 datastreams exist per site
                ds_key = f'Datastreams/{i}'
                ds_name = properties.get(f'{ds_key}/name')
                ds_id = properties.get(f'{ds_key}/@iot.id')

                if not ds_id:
                    continue  # Skip missing datastreams

                # Observed Property
                op_name = properties.get(f'{ds_key}/ObservedProperty/name')
                op_description = properties.get(f'{ds_key}/ObservedProperty/description')

                # Observations
                obs_result = properties.get(f'{ds_key}/Observations/0/result')
                obs_time = properties.get(f'{ds_key}/Observations/0/phenomenonTime')
    ...
    # Convert datatypes
    #gdf["obs_result"] = pd.to_numeric(gdf["obs_result"], errors='coerce')
    #gdf["obs_time"] = pd.to_datetime(gdf["obs_time"], errors='coerce').dt.tz_convert("US/Pacific")
    #gdf.assign(day_of_year=lambda x: x['obs_time'].dt.dayofyear)  # Extract day of the year
    return gdf
...
def geolocate_usgs_sensors(sensor_gdf):
    ...
    geojson_path = os.path.join(script_directory, 'Data', 'NHD_Named_CA_Rivers.geojson')
    NHD_rivers_gpd = (
...
def add_percentile_streamflows(gdf):
    percentile_df = (pd
                    .read_csv(os.path.join(script_directory, 'Data', 'streamflow_percentiles.csv'),
                    dtype= {'site_no': 'str'})
    )
    merged_gdf = (gdf
                  .merge(percentile_df, on= ['site_no', 'day_of_year'], how = 'left')        
    )
...
def append_new_sensor_data_to_postgis(gdf):
    ...
    Change username, password and host settings based on credentials where db is hosted.
    """
    # Server credentials
...
def main():
    ...
    try:
        ...
        if response:
            ...
            current_streamflow_gage_gdf = merge_streamflow_gage_height_gdfs(streamflow_filtered_gdf, gage_height_filtered_gdf).drop(columns = 'op_name')
            ...
            print(percentiles_added_gdf.columns)
            print(percentiles_added_gdf.dtypes)
            print(percentiles_added_gdf.isna().sum())
